...
| `IAuditLog` | Solo `receipt.audit` JSON embebido | Docs/02 ya nombraba `audit_events`; no existía la tabla |
...
## Auditoría (esquema v2)

`IAuditLog` es la tabla `audit_events`. `receipts.audit_json` se conserva como recorte por comprobante. Al abrir un `.sqlite` v1, se rellena `audit_events` desde ese JSON.
